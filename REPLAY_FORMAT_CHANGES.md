...
**注意**: 观测空间已更新为29维（详见`观测空间说明.md`），本文档已同步更新。

## 主要问题（修改前）

1. **没有保存观测数据**：replay中完全没有保存observation向量
2. **缺少游戏seed**：无法重现相同的游戏状态
3. **动作记录不精确**：只保存card_value，无法精确记录使用的卡牌
...
```json
{
  ...
  "transitions": [               // 完整的transition数据
    {
      "step": 0,
      "observation": [float × 29],           // 29维观测向量
      "valid_actions": [bool × 6],           // 6维动作掩码
      "action": 2,                           // 实际选择的动作索引（0-5）
      "action_type": "move",                 // 'move' 或 'collect'
      "card_value": 3,                       // 卡牌点数（1-3）
      "reward": 0.0,                         // 本步获得的奖励
      "next_observation": [float × 29],      // 下一步的观测
      "done": false,                         // 是否游戏结束
      "info": {                              // 额外信息
...
详细说明请参考 `观测空间说明.md`，简要结构如下：

- 手牌统计（3维）：1点、2点、3点各有几张
- 位置（10维，one-hot）：环形地图位置
- 资源系数（1维）：当前资源系数
- 当前回合（1维）：回合进度
- 可普通收集（1维）：是否可以普通收集
- 可连击（1维）：是否可以连击收集
- 上次收集代价（2维，one-hot）：区分收集1和收集2
- 顾客需求（9维）：3个顾客各需的冰/铁/火量
- 代币数（1维）：当前代币数
...
- 0-2：移动动作（使用1点、2点、3点牌进行移动）
- 3-5：收集动作（使用1点、2点、3点牌进行收集）
...
1. 创建游戏实例并模拟几步
2. 验证transition的数据结构
3. 检查observation维度（应为29）
4. 检查valid_actions维度（应为6）
5. 检查action索引范围（应为0-5）
...
| 特性 | 修改前 | 修改后 |
|------|--------|--------|
| 保存observation | ✗ | ✓ 完整的29维向量 |
| 保存动作掩码 | ✗ | ✓ 6维布尔数组 |
| 保存seed | ✗ | ✓ 可重现游戏 |
| 动作记录精确性 | ✗ 只有card_value | ✓ 完整的card_value和action索引 |
| 可用于监督学习 | ✗ | ✓ |
| 可用于离线强化学习 | ✗ | ✓ |
| 可用于行为克隆 | ✗ | ✓ |
